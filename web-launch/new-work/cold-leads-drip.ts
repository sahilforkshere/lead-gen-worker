// ═══════════════════════════════════════════════════════════════════════════════
// EDGE FUNCTION: cold-leads-drip (GOOGLE MAPS LOCAL PIPELINE)
//
...
serve(async (req) => {
  ...
  try {
    ...
    for (const place of places) {
      ...
      coldLeads.push({
        // 🚀 NEW TOP-LEVEL COLUMNS (For Dashboard UI)
        company_name:  name,
        phone:         phone,
        address:       address,
        listing_url:   permanentMapsUrl,
        place_id:      placeId || null,             // Added for map compliance/unique identification
        rating:        place.totalScore || null,    // Added for UI Badges
        email:         email,                       // Added to evaluate UI Tiers instantly
        ...
        lead_data: {
          ...
          tier:     email ? "tier_1_enriched" : phone ? "tier_2_phone_only" : "tier_3_none",
