...
const MAX_DOMAIN_AGE_DAYS         = 21;   // Domains & Subdomains Discovery: look back 7 days for freshest leads
...
const NICHE_KEYWORDS: Record<string, string[]> = {
  ...
  "pest control": ["pest", "bug", "termite", "exterminat"], // 🚀 ADDED & SHORTENED
...
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// STEP 2: websitelaunches.com SINGLE LOOKUP (Free Tier Bypass)
// Returns only domains where launch_detected = false AND authority <= threshold
// ─────────────────────────────────────────────────────────────────────────────
// ─────────────────────────────────────────────────────────────────────────────
// STEP 2: websitelaunches.com SINGLE LOOKUP (Free Tier Bypass)
...
async function checkUnbuiltDomains(domains: string[]): Promise<{
  ...
  if (!WEBSITELAUNCHES_KEY) {
    console.warn("   [WebsiteLaunches] ⚠️ No API key — skipping filter.");
    return [];
  }

  const batch = domains.slice(0, 25);
  console.log(`   [WebsiteLaunches] Checking ${batch.length} domains individually (Free Tier Mode)…`);

  const unbuilt = [];

  for (const d of batch) {
    try {
      // 🚀 FIX 1: URL is now a direct path (/domain/apple.com)
      ...
      if (res.status === 429) {
        console.warn(`   [WebsiteLaunches] ⚠️ Rate limited! Stopping checks early.`);
        break; 
      }
      ...
      const r = data?.data; // The actual domain info lives inside the "data" object

      if (!r) continue;

      // 🚀 FIX 2: Check for "site_authority" based on new API docs
      ...
      // Sleep for 500ms between requests to protect your API limits
      await new Promise(resolve => setTimeout(resolve, 500));

    } catch (err) {
      console.error(`   [WebsiteLaunches] ❌ Error checking ${d}: ${(err as Error).message}`);
    }
  }

  console.log(`   [WebsiteLaunches] ✅ ${batch.length} checked → ${unbuilt.length} confirmed strictly unbuilt!`);
  return unbuilt;
}
...
serve(async (req) => {
  ...
  try {
   ...
   // ── STEP 1: Collect keywords for this niche ─────────────────────────────
    ...
    // If the niche from the roulette wheel isn't in our dictionary, create a dynamic keyword!
    if (!rawKeywords) {
      // Grab the first word of the niche (e.g., "massage therapist" -> "massage")
      const dynamicKeyword = niche.trim().split(" ")[0].toLowerCase();
      rawKeywords = [dynamicKeyword];
      console.log(`   ⚠️ Niche '${niche}' not in dictionary. Using dynamic keyword: [${dynamicKeyword}]`);
    }

 // 🚀 THE FIX: Only send 1 keyword to prevent the API's "AND" condition trap.
