...
// ── 1. APOLLO FETCHER (Handles 429s and 422s safely)
...
serve(async (req) => {
  ...
  try {
    ...
    // ── Phase 2: Format Data (NO INBUILT SCANNING) ───────────────────────────
    const rawRows: any[] = [];
    const cleanRows: any[] = [];
    ...
    companies.forEach((company: any) => {
      ...
      cleanRows.push({
        ...
        pitch_angle: "Pending Tech Stack Analysis", // Placeholder until RapidAPI runs
        has_website: true,
        status: "pending", // Keeps it ready for the next function
        lead_data: {
    ...
    // ── Phase 4: Complete Job & Trigger RapidAPI ─────────────────────────────
    if (preference_id) {
      await supabase.from("user_preferences")
        .update({ status: "completed", completed_at: new Date().toISOString() })
        .eq("id", preference_id);
      
      console.log(`   🚀 Firing detect-tech-stack in the background...`);
      // 🎯 NEW: Fire-and-forget call to your new RapidAPI tech scanner
      fetch(`${supabaseUrl}/functions/v1/detect-tech-stack`, {
        method: "POST",
        headers: {
      ...
      }).catch((e) => console.error("   detect-tech-stack fire error:", e.message));
    }
